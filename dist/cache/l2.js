...
async function flushQueue() {
    ...
    for (const it of q) {
        ...
        try {
            if (it.payload.length > compressionThreshold) {
                payloadBuf = deflateSync(it.payload);
                compressed = true;
            }
        }
        catch (_) {
            payloadBuf = it.payload;
            compressed = false;
        }
        const metaBuf = msgpackEncode({ compressed, serializer: 'msgpackr' });
        if (it.ttl && it.ttl > 0) {
